...
public class UIModel {
    ...
    // --- Added: variables for login attempt limiting ---
    // These aren't actively used to drive logic here (that is handled in BankAccount)
    // but kept as a record of the intended max and to track attempts at the UI level
    ...
    private void reset(String msg) {
        setState(STATE_ACCOUNT_NO);
        numberPadInput = "";
        // --- Added: also clear the stord account number and password on reset ---
        // Without this, old values could linger and cause unexpected behavior on re-login
        accNumber = "";
        accPasswd = "";
        message = msg;
        result = "Enter your account number\nFollowed by \"Ent\"";
        ...
        switch ( state )
        {
            case STATE_ACCOUNT_NO:
                ...
                if (numberPadInput.equals("")) {
                    ...
                    // --- Added: validate the account exists and isn't locked before proceeding ---
                    // In the template, any account number was accepted here
                    // Now we check the bank first so the user gets immediate feedback
                    BankAccount acc = bank.getAccount(accNumber);
                    ...
                    if (acc.isLocked()) {
                        message = "Account locked. Try again tomorrow";
                        reset(message);
                        break;
                    }

                    // Only advance to password input if account number is valid and account is not locked
            ...
            case STATE_PASSWORD:
                ...
                // --- Added: re-check the account status before attempting login ---
                // Needed in case the account became locked between account entry and password entry
                BankAccount acc = bank.getAccount(accNumber);
                ...
                if (bank.login(accNumber, accPasswd)) {
                    ...
                    // --- Added: show how many attempts have been used instead of a generic fail message ---
                    // Checks how many failed attempts there will be after this attempt
                    int attempts = acc.getFailedAttempts();

                    if (attempts == 3) {
                        message = "Password incorrect. Account blocked";
                        reset(message); // Kicks back to login screen
                    } else {
                        message = "Login failed (" + attempts + "/3)";
        ...
        if (state.equals(STATE_LOGGED_IN)) {
            ...
            if (amount > 0) {
                if(bank.withdraw( amount )){
                    // --- Changed: show the account's transaction message and new balance ---
                    // The template just showed "Withdraw Successful", now it pulls the actual
                    // message from the account (which varies by account type) and shows the new balance
                    message = bank.getLoggedInAccount().getLastTransactionMessage();
                    result = "New balance: " + bank.getBalance();
                }
                else{
                    message = "Withdraw Failed: " + bank.getLoggedInAccount().getLastTransactionMessage();
                    result = "Now enter the amount\nThen press transaction\n(Dep = Deposit, W/D = Withdraw)";
                }
            }
            else{
    ...
    public void processDeposit() {
        if (state.equals(STATE_LOGGED_IN)) {
            ...
            if (amount > 0) {
                bank.deposit( amount );
                // --- Changed: show the account's transaction message and new balance ---
                // Same reasoning as processWithdraw above
                message = bank.getLoggedInAccount().getLastTransactionMessage();
                result = "New balance: " + bank.getBalance();
            }
            else {
    ...
    // --- Added: handle the "Day" button ---
    // Calls bank.nextDay() which resets limits and applies interest across all accounts
    // Updates the display to confirm a new day has started
    public void processNextDay() {
        bank.nextDay();
        message = "New day started";
        result = "Limits and locks reset, interest applied";
        update();
    }
