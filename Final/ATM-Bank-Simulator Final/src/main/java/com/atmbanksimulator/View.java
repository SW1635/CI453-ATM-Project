...
class View {
    ...
    int W = 510;         // Width  of window pixels
    ...
    public void start(Stage window) {
        ...
        String buttonTexts[][] = {
                ...
                {"4",    "5",  "6",  "",  "W/D",  ""},
                {"1",    "2",  "3",  "",  "Bal",  "Fin"},
                // --- Added: "Day" button in bottom row ---
                // The template had an empty space here, "Day" was added to trigger the next day simulation
                {"CLR",  "0",  "",   "",  "Day",  "Ent"} };
        ...
        for ( String[] row: buttonTexts ) {
            for (String text: row) {
                if ( text.length() >= 1 ) {
                    ...
                    // --- Added: colour code key buttons so the UI looks better and is easier to use ---
                    // The template had no button styling, these "if" blocks apply different
                    // background colours to make Dep/W/D/Bal/CLR, Ent, Fin and Day visually distinct
                    if (text.equals("Dep") || text.equals("W/D") || text.equals("Bal") || text.equals("CLR")) {
                        btn.setStyle("-fx-background-color: #2e5557; -fx-text-fill: #cddcd1;");
                    }

                    if (text.equals("Ent")) {
                        btn.setStyle("-fx-background-color: #429a79; -fx-text-fill: white;");
                    }

                    if (text.equals("Fin")) {
                        btn.setStyle("-fx-background-color: #74453e; -fx-text-fill: white;");
                    }

                    if (text.equals("Day")) {
                        btn.setStyle("-fx-background-color: #cddcd1; -fx-text-fill: #291f1f;");
                    }
